  ...
  <div class="overflow-x-auto">
    <table class="w-full text-left">
      ...
      <tbody>
        ...
        <tr class="border-t hover:bg-gray-50 transition-colors" data-booking-id="{{ $b->id }}">
          ...
          <td class="p-3 text-sm text-gray-900">{{ $b->driver_name ?? '-' }}</td>
          @if(isset($active) && in_array($active, ['confirmed','completed']))
            <td class="p-3">
              @php
          ...
          @endif
          <td class="p-3 text-sm text-gray-900">{{ $b->driver_price ? number_format($b->driver_price,2) : '-' }}</td>
          <td class="p-3 text-sm text-gray-900">{{ $b->total_price ? number_format($b->total_price,2) : '-' }}</td>
          <td class="p-3">
        ...
        @empty
        <tr>
          <td colspan="{{ isset($active) && in_array($active, ['confirmed','completed']) ? '11' : '10' }}" class="p-8 text-center text-gray-500">
