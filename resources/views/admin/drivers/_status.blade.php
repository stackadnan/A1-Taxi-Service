    ...
    <div class="overflow-x-auto">
      ...
      <tbody>
        ...
        <tr class="border-t" data-driver-id="{{ $d->id }}">
          ...
          <td class="p-2">
            @if($d->current_booking && $d->status_label === 'POB')
              <button 
                onclick="startInlineTracking({{ $d->id }}, {{ $d->current_booking->id }}, '{{ $d->name }}', '{{ $d->current_booking->booking_code }}', 'POB')" 
...
(function(){
  ...
  function updateDriverLocation() {
    ...
    // For non-POB drivers, use a simplified endpoint that only fetches driver location
    ...
    .then(data => {
      ...
      // For POB drivers, show pickup and destination from booking
      if (isPOB) {
        ...
        if (shouldRecalculateRoute && !isNaN(destLat) && !isNaN(destLng)) {
