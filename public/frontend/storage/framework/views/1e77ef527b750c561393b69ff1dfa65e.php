...
var GOOGLE_MAPS_API_KEY=<?php echo json_encode($googleMapsKey, 15, 512) ?>;
var VEHICLES=[ 
...
function logAirportChargeSource(){
  var pickup = (qd.pickup || '').toLowerCase();
  var dropoff = (qd.dropoff || '').toLowerCase();
  var charge = Number(qd.airport_charges || 0);
  var pickupAirport = /airport/.test(pickup);
  var dropoffAirport = /airport/.test(dropoff);
  if (charge > 0) {
    console.log('Airport charges debug: £' + charge);
    if (pickupAirport && dropoffAirport) {
      console.log('Airport charges added at both pickup and dropoff.');
    } else if (pickupAirport) {
      console.log('Airport charges added at pickup location.');
    } else if (dropoffAirport) {
      console.log('Airport charges added at dropoff location.');
    } else {
      console.log('Airport charges added, but pickup/dropoff addresses do not include "airport" text.');
      console.log('Pickup:', qd.pickup);
      console.log('Dropoff:', qd.dropoff);
    }
  } else {
    console.log('No airport charges added for this quote. Pickup:', qd.pickup, 'Dropoff:', qd.dropoff);
  }
}
    ...
    '<div class="vcard-grid">'+
      ...
      '<div class="vc-pricing">'+
        '<div class="price-card">'+
          ...
          '<button class="btn btn-ow" data-book="1" data-vehicle="'+h(v.key)+'" data-vehicle-name="'+h(v.name)+'" data-trip="one-way" data-price="'+(os||'')+'" '+owDisabled+'><i class="fa-solid fa-arrow-right"></i> Select one-way</button>'+ 
        '</div>'+

        '<div class="price-card">'+
          '<div class="p-lbl">Total return price</div>'+
          '<div class="p-amount">'+(rs?'<span class="sym">£</span>'+rs:'N/A')+'</div>'+ 
          '<button class="btn btn-ret" data-book="1" data-vehicle="'+h(v.key)+'" data-vehicle-name="'+h(v.name)+'" data-trip="return" data-price="'+(rs||'')+'" '+retDisabled+'><i class="fa-solid fa-rotate"></i> Select return</button>'+
        '</div>'+
      '</div>'+
...
document.addEventListener('click', function(e) {
  ...
  if (b && !b.disabled) {
    doBook(b.dataset.vehicle, b.dataset.vehicleName || b.dataset.vehicle, +b.dataset.price, b.dataset.trip);
    return;
  }
