...
class ManageBookingController extends Controller
{
    ...
    private function sendBookingEmails(int $bookingId, string $bookingCode, array $validated, ?int $returnBookingId = null, ?string $returnBookingCode = null): void
    {
        ...
        $viewData = [
            ...
            'returnBookingCode' => $returnBookingCode,
            'returnBookingId' => $returnBookingId,
            'basePriceText' => $basePriceText,
            'meetAndGreetText' => $meetAndGreetText,
            'vatText' => $vatText,
            'vatRateText' => $vatRateText,
            'price' => $price,
            'isReturnTrip' => $isReturnTrip,
            'companyWebsite' => rtrim((string) config('app.url', '#'), '/'),
        ];

        try {
            Mail::send('email.booking-confirmation-mail-to-admin', array_merge($viewData, [
                'recipientName' => 'Admin',
                'userEmail' => $userEmail,
            ]), function ($message) use ($adminEmail, $bookingCode): void {
        ...
        try {
            Mail::send('email.booking-confirmation-mail-to-customer', array_merge($viewData, [
                'recipientName' => $passengerName,
                'userEmail' => $userEmail,
            ]), function ($message) use ($userEmail, $bookingCode): void {
                $message
                    ->to($userEmail)
